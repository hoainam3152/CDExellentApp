...
namespace CDExellentAPI.Controllers
{
    ...
    {
        private readonly IQuestionRepository service;
        ...
        [HttpPost]
        public IActionResult Create(QuestionCreate request)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        [HttpPut]
        public IActionResult Update(int id, QuestionUpdate request)
        {
            if (ModelState.IsValid)
            {
                try
